...
export default function ProdutosPainel() {
  ...
  return (
    <div className="max-w-6xl mx-auto mt-10 p-4 bg-white rounded-xl shadow-lg border border-gray-200">
      ...
      {toast.show && (
        <div
          className={`fixed top-6 left-1/2 -translate-x-1/2 px-6 py-3 rounded shadow-lg bg-white border border-gray-300 text-center z-50 text-${toast.color}-600 font-semibold`}
        >
          {toast.text}
        </div>
      )}
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
        {produtosFiltrados.map((produto) => (
          <div
            key={produto.id}
            className="bg-white border border-gray-100 rounded-2xl shadow-lg p-6 flex flex-col items-start transition hover:scale-[1.02]"
          >
            <img
            ...
            />
            <h3 className="text-lg font-bold mb-1 text-[#7b1e3a] font-serif">
              {produto.nome}
            </h3>
            <div className="mb-1 text-xs text-gray-500">
              {produto.categorias?.nome}
            </div>
            <div className="mb-1 text-gray-700 font-sans">
              {produto.descricao}
            </div>
            ...
            <div className="flex gap-4 mt-2 items-center">
              <Switch
                ...
                onChange={() =>
                  alterarDisponibilidade(produto.id, produto.disponivel)
                }
      ...
      {modal.show && (
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-lg p-6 w-full mx-4 max-w-sm text-center border border-gray-300">
            <h4 className="text-lg font-bold mb-2 text-[#7b1e3a]">
              Confirmar exclusão
            </h4>
